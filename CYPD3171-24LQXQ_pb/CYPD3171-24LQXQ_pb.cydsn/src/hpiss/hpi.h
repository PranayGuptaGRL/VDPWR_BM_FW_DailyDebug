...
/**< UCSI Control Register Signal Connnect Event value. EC sends this command to ask CCG to send connect
     event information to OS. */

#endif /* CCG_UCSI_ENABLE */

#define HPI_PD_CMD_TIMER                (0x80)
/**< Timer ID used for command retry checks. */
...
/**
 * @typedef hpi_auto_cb_t
 * @brief Handler for HPI Auto specific register.
 * @return Type of response to be sent to the EC. Only a single byte response
 * can be sent from here.
 */
typedef uint8_t (*hpi_auto_cb_t)(
        uint16_t  command,             /**< Command value */
        uint8_t   value                /**< Associated command value. */
        );

/**
 * @typedef_auto_config_tt
...
#if CCG_HPI_AUTO_CMD_ENABLE
/**
 * @brief Enable handling of HPI Auto defined register writes in the Application.
 *
 * This function is used to enable handling of EC writes to the HPI Auto defined
 * register region in the application code.
 *
 * @param wr_handler Pointer to function that handles the received HPI writes.
 */
void hpi_set_auto_cb_handler (hpi_auto_cb_t wr_handler);

/**
 * @brief Enable updating for Auto specific firmware parameters to be send.
 *
 * This function is used to update Auto specific firmware parameters to device
 * Flash memory section. This function should get triggered in response to
 * HPI_GET_AUTO_FW_CONFIG_CMD command.
 *
 * @param data Auto data to send.
 */
void hpi_auto_set_config(hpi_auto_config_t data);
