...
#if ((DELAYED_TBT_LSXX_CONNECT) && ((TBT_DFP_SUPP) || (TBT_UFP_SUPP)))
...
{
    uint8_t app_sbu_conf = pd_get_ptr_tbthost_cfg_tbl(port)->sbu_config;

    if (app_sbu_conf == HOST_SBU_CFG_PASS_THROUGH)
    {
    ...
    {
        if ((app_sbu_conf == HOST_SBU_CFG_FULL) && (dpm_get_info(port)->polarity))
        {
...
#endif /* ((DELAYED_TBT_LSXX_CONNECT) && ((TBT_DFP_SUPP) || (TBT_UFP_SUPP))) */
...
void ridge_slave_task(void)
{
    ...
    if (ridge_slave_task_pending)
    {
        ...
        switch (ridge_slave_write_buf[0] >> 1)
        {
            ...
            default:
                /* Re-enable slave ACK. */
                CALL_IN_FUNCTION(i2c_slave_ack_ctrl)(ridge_slave_scb_idx, false);
 ...
 */
bool ridge_slave_i2c_cmd_callback(i2c_cb_cmd_t cmd, i2c_scb_state_t i2c_state, uint16_t count)
{
    bool retval = false;
    uint16_t size;
    uint8_t temp, port = 0;
    ...
    if (cmd == I2C_CB_CMD_WRITE)
    {
        ...
        CALL_IN_FUNCTION(i2c_slave_ack_ctrl)(ridge_slave_scb_idx, true);
    ...
    if (cmd == I2C_CB_CMD_READ)
    {
        ...
        if ((ridge_slave_read_ptr != 0) && (ridge_slave_read_ptr < ridge_slave_read_limit))
        {
            ...
            if (size != 0)
            {
                CALL_IN_FUNCTION(i2c_scb_write)(ridge_slave_scb_idx, (uint8_t *)ridge_slave_read_ptr,
                        (uint8_t)size, &temp);
...
static void ridge_slave_data_reset(void)
{
...
#else
    CALL_IN_FUNCTION(i2c_scb_init)(ridge_slave_scb_idx, I2C_SCB_MODE_ALP_RIDGE, RIDGE_SLAVE_SCB_CLOCK_FREQ,
            ((portsel != 0) ? RIDGE_SLAVE_ADDR_P1 : RIDGE_SLAVE_ADDR_P0), I2C_SLAVE_ADDR_MASK_DEFAULT,
            ridge_slave_i2c_cmd_callback, ridge_slave_write_buf, RIDGE_SLAVE_MAX_WRITE_SIZE);
#endif /* CCG_PD_DUALPORT_ENABLE */

    /* Clear all data structures and de-assert interrupts. */
    ridge_slave_data_reset ();
    ridge_slave_enabled = true;
}
...
void ridge_slave_deinit(void)
{
    ...
    /* De-init the I2C-slave used for the interface. */
    CALL_IN_FUNCTION(i2c_scb_deinit)(ridge_slave_scb_idx);

    /* Mark block disabled. */
    ridge_slave_enabled = false;
}
...
bool ridge_slave_sleep(void)
{
    ...
    /* Perform other checks only if the block is enabled. */
    if (ridge_slave_enabled)
    {
        /* Sleep is not if the I2C interface is busy. */
        if ((ridge_slave_task_pending) || (!(CALL_IN_FUNCTION(i2c_scb_is_idle)(ridge_slave_scb_idx))))
        {
            stat = false;
        }
        else
        {
            ...
            CALL_IN_FUNCTION(i2c_scb_enable_wakeup)(ridge_slave_scb_idx);
        }
    }
