...
#if (defined CCG5C) || (defined CCG6)
    bc_fsm_src_apply_cdp,                       /*  4: BC_FSM_SRC_APPLY_CDP */
#else
    bc_fsm_off,                                 /*  4: BC_FSM_SRC_APPLY_CDP */
#endif /* (defined CCG5C) || (defined CCG6) */
...
//        chgb_apply_sink_term (c_port, CHGB_SINK_TERM_SCD);
    ...
    else if(evt == BC_FSM_EVT_TIMEOUT1)
    {
        ...
        {
            /* CDP connected. */
            bc_stat->cur_mode = BC_CHARGE_CDP;//Pranay,08Aug'19, Added as per Rajath's comments
