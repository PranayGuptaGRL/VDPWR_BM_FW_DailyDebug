...
#if DP_DFP_SUPP                    
                       || ((am_info_p->VDM_HDR.svid == DP_SVID) && (am_info_p->vdo[SOP]->val == DP_USB_SS_CONFIG) && (am_info_p->VDM_HDR.cmd == DP_STATE_CONFIG))
#endif /* DP_DFP_SUPP */                    
                   )
                {
...
#if DFP_ALT_MODE_SUPP
bool is_enter_allowed(uint8_t port, uint8_t id, uint16_t svid)
{
    ...
    if (alt_mode[port].am_active_modes != NO_DATA)
    {
        for (idx = 0; idx < am_numb; idx++)
        {
            ...
            if (IS_FLAG_CHECKED(alt_mode[port].am_active_modes, idx))
            {
                ...
                for (idx2 = 0; idx2 < am_numb; idx2++)
                {
                    tbl_addr = &dfp_compatibility_mode_table[idx][idx2];
                    if ((tbl_addr->svid == svid) && (tbl_addr->alt_mode_id == id))
                    {
                        am_allowed = true;
                    }
                }
        ...
        if (cmd_info.alt_mode_event.alt_mode_evt == AM_CMD_ENTER)
        {
            ...
            if (found)
            {
                if (
                        (am_info_p->cbk != NULL) && 
                        (am_info_p->is_active == false) && 
                        (is_enter_allowed(port, am_info_p->VDM_HDR.svid, am_info_p->alt_mode_id) != false)
                    )
                {
