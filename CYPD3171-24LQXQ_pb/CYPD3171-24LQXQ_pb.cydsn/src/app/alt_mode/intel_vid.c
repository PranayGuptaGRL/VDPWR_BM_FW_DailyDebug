...
static void generate_enter_mode_vdo(uint8_t port, pd_do_t svid_vdo, pd_do_t cable_vdo)
{
       ...
       )
    {
        ...
        {
            tbt[port].enter_mode_vdo.tbt_vdo.vpro_dock_host = 1;
        }
    }
...
static tbt_state_t eval_tbt_attention(uint8_t port)
{
    ...
    /* If Exit is needed */
    if (TBT_EXIT(att->val) || USB2_ENABLE(att->val))
    {
        stat = TBT_STATE_EXIT;
    }
...
static bool tbt_set_regs_enter(uint8_t port)
{
    ...
    reg->ridge_stat.pro_dock_detect = tbt_param->tbt_vdo.vpro_dock_host;
